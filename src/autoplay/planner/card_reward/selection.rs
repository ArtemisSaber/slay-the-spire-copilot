...
use super::{retry, structured_scenario};

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::autoplay::planner) struct CardCandidateSelection {
    pub(in crate::autoplay::planner) candidate_index: usize,
    pub(in crate::autoplay::planner) choice_index: usize,
...
pub(super) async fn select_best_card(
    ...
    let base_prompt = build_card_candidate_prompt(
        session,
        state,
        locale,
        shop_visited,
    ...
    for attempt in 1..=retry::MAX_STAGE_ATTEMPTS {
        let prompt = retry::prompt_for_attempt(&base_prompt, attempt, last_error.as_ref())?;
        let result = match provider.query_card_reward_candidate(&prompt).await {
            Ok(response) => {
                parse_card_candidate_response(&response, candidates, allowed_memory_ids)
            }
            Err(error) => Err(error.context("card candidate selector query failed")),
        };
        match result {
            Ok(selection) => return Ok(selection),
            Err(error) => {
                tracing::warn!("card candidate selector attempt={attempt} rejected: {error}");
                last_error = Some(error);
            }
        }
    }
    Err(retry::exhausted("card candidate selector", last_error))
}
...
pub(in crate::autoplay::planner) fn build_card_candidate_prompt(
    session: &AutoPlaySession,
    state: &NormalizedState,
    locale: &Locale,
    shop_visited: bool,
...
pub(in crate::autoplay::planner) fn parse_card_candidate_response(
    response: &str,
    candidates: &[ActionCandidate],
    allowed_memory_ids: &[String],
) -> anyhow::Result<CardCandidateSelection> {
