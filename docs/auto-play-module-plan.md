...
| `prompt.rs` | Build executor-oriented prompts with stable action IDs and indices. |
| `planner.rs` | Ask LLM for an action or action sequence; no stdout writes. |
| `resolver.rs` | Map chosen action IDs to current game-state choices, hand indices, targets, or map nodes. |
...
Action IDs should be generated by the prompt builder and echoed by the LLM. This avoids fuzzy matching localized card names and duplicate cards.

Examples:

| Screen | Prompt action IDs |
|---|---|
...
- Action parser rejects invalid JSON, unknown action IDs, and ambiguous choices.
