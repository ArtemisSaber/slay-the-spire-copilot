...
|---|---|---|
| `ref` | yes | Prompt-scoped reference copied from one `available_actions` entry. |
| `target_index` | targeted actions only | Existing monster index; omitted for untargeted actions. |
...
- Invalid JSON syntax.
- Valid JSON that does not match the planner envelope.
- Unknown schema versions.
- Empty actions.
- More than one action on screens that support only one safe action.
- Unknown or non-canonical `ref`.
- Missing targets for targeted actions or targets supplied to untargeted actions.
...
Internal action IDs hidden behind prompt refs:
